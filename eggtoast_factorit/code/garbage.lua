gui = {
    action = function(x,y,b) -- DELETEME: the code is ass
        ...
        if util.nullcheck(tbl) then
            for _, tbl in pairs(tbl) do
                ...
                if util.nullcheck(tbl.a) and util.boxcol(x,y,sx,sy,mx,my) then
                    tbl.a()
                end
            end
            if util.nullcheck(tbl) then
        ...
        if util.nullcheck(t) then
            ...
            for k, this in pairs(t) do
                ...
                if util.nullcheck(k) and (type(k) ~= "string") and not ((k == "g") or (k == "global")) then
                    ...
                    local neon = function(a,b)
                        if util.nullcheck(this[a]) then
                            this[b] = this[a]
                        end
                    end
                    local ne = function(a,b)
                        if not util.nullcheck(this[a]) then
                            this[a] = b
                        end
                    end
                    ...
                    if util.nullcheck(this.b) and #this.b ~= 5 then
                        if util.nullcheck(this.b[4]) then
                            this.b[5] = this.b[4]
                        else
                            --this.b[5] = 
                        end
                    end
                    if not util.nullcheck(this.is) then
                        this.is = this.s
                    end
                    ne("st",nil);ne("sl",nil)
                    ne("bs",false)
    ...
    ---@diagnostic disable-next-line: cast-local-type
                    x, y, sx, sy = neo(x), neo(y), neo(sx), neo(sy)
                    x, y, sx, sy = gui.script.func[x[1]](x[2]), gui.script.func[y[1]](y[2]), gui.script.func[sx[1]](sx[2]), gui.script.func[sy[1]](sy[2])
                    if util.nullcheck(this.bg) then
                        setc(this.bg)
                    ...
                    if util.nullcheck(this.r) then
                        this.r()
                    end
                    if util.nullcheck(this.i) then
